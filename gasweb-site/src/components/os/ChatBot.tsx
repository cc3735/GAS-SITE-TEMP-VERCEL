...
const ChatBot: React.FC = () => {
  ...
  const getBotResponse = (userMessage: string): string => {
    ...
    if (message.includes('project') || message.includes('projects')) {
      ...
      if (message.includes('delete') || message.includes('remove')) {
        return 'To delete a project, click the trash icon next to the project tab. You\'ll get a confirmation dialog. All associated tasks and data will be removed permanently.';
      }
  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 transition-all duration-200 ${isMinimized ? 'w-80 h-14' : 'w-80 h-96'}`}>
      <div className="bg-gray-900 rounded-lg shadow-xl border border-gray-700 flex flex-col h-full">
        {/* Header */}
            ...
            <div className="border-t border-gray-700 p-4">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none text-sm bg-gray-800 text-white placeholder-gray-500"
                />
                <button
