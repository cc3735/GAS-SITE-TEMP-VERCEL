...
const AgentManagementModal: React.FC<AgentManagementModalProps> = ({
  ...
  const getAgentTypeInfo = (type: string) => {
    const types = {
      'content_generation': { name: 'Content Generation', icon: '✏️', color: 'bg-purple-900/30 text-purple-300' },
      'voice': { name: 'Voice Agent', icon: '📞', color: 'bg-green-900/30 text-green-300' },
      'operating': { name: 'Operating Agent', icon: '🧠', color: 'bg-blue-900/30 text-blue-300' },
      'social_media': { name: 'Social Media Bot', icon: '📱', color: 'bg-pink-900/30 text-pink-300' },
      'lead_qualification': { name: 'Lead Qualification', icon: '🎯', color: 'bg-orange-900/30 text-orange-300' }
    };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-xl max-w-6xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-700 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{agentTypeInfo.name}</p>
            </div>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition ${
                      activeTab === tab.id
                        ? 'bg-blue-900/20 border-r-2 border-primary-600 text-blue-400'
            ...
            {isEditing && (
              <div className="mt-6 pt-6 border-t border-gray-700">
                <button
                  ...
                  className="w-full flex items-center gap-2 px-4 py-2 text-red-400 hover:text-red-300 border border-red-700 hover:border-red-600 rounded-lg transition"
                >
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            {activeTab === 'overview' && (
              <div className="space-y-6">
                ...
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      {isEditing ? (
                        <input
                          ...
                          className="w-full px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-gray-800 text-white placeholder-gray-500"
                        />
                      ) : (
                    ...
                    <div>
                      ...
                      <p className="text-white">{agentTypeInfo.name}</p>
                    </div>
                    <div className="md:col-span-2">
                      ...
                      {isEditing ? (
                        <textarea
                          ...
                          rows={3}
                          className="w-full px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none resize-none bg-gray-800 text-white placeholder-gray-500"
                        />
                      ) : (
                ...
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-primary-400">{agent.performance_metrics?.tasks_completed || 0}</div>
                      <div className="text-sm text-gray-400">Tasks Completed</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-400">{agent.performance_metrics?.success_rate || 0}%</div>
                      <div className="text-sm text-gray-400">Success Rate</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-purple-400">{agent.performance_metrics?.tokens_used || 0}</div>
                ...
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Connected MCP Servers</h3>
                  <div className="flex flex-wrap gap-2">
                    {formData.mcpServerIds.length > 0 ? (
                      formData.mcpServerIds.map(serverId => {
                        const server = mcpServers.find(s => s.id === serverId);
                        return (
                          <span key={serverId} className="px-3 py-1 bg-gray-800 text-gray-200 rounded-full text-sm">
                    ...
                    <div>
                      ...
                      {isEditing ? (
                        <select
                          ...
                          className="w-full px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-gray-800 text-white"
                        >
                        ...
                        <p className="text-white">{formData.aiModel}</p>
                      )}
                    </div>
                    <div>
                      ...
                      {isEditing ? (
                        <div>
                          <input
                            ...
                            className="w-full"
                          />
                          <span className="text-sm text-gray-400 block mt-1">{formData.temperature}</span>
                        </div>
                      ) : (
                        <p className="text-white">{formData.temperature}</p>
                      )}
                    </div>
                    <div className="md:col-span-2">
                      ...
                      {isEditing ? (
                        ...
                        <p className="text-white">{formData.maxTokens}</p>
                      )}
                    </div>
                  </div>
                </div>
                      ...
                      {mcpServers.map(server => (
                        <div key={server.id} className="flex items-center gap-3">
                          ...
                          <label htmlFor={server.id} className="text-sm">
                            ...
                            <span className="text-gray-400 ml-2">({server.server_type})</span>
                          </label>
                        </div>
                      ))}
                      {mcpServers.length === 0 && (
            ...
            {activeTab === 'resources' && (
              <div className="space-y-6">
                {/* Prompt Template */}
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Custom Prompt Template</h3>
                  {isEditing ? (
                    <textarea
                      ...
                      className="w-full px-4 py-3 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none font-mono text-sm bg-gray-800 text-white placeholder-gray-500"
                    />
                  ) : (
                ...
                {/* Reference Links */}
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Reference Links</h3>
                  <div className="space-y-3">
                      ...
                      <div key={link.id} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <a href={link.url} target="_blank" rel="noopener noreferrer" className="font-medium text-primary-400 hover:underline truncate">
                              {link.title}
                            </a>
                          </div>
                          {link.description && (
                    ...
                    {isEditing && (
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4">
                        <input
                          ...
                          className="px-3 py-2 border border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none bg-gray-800 text-white placeholder-gray-500"
                        />
                        <input
                          type="text"
                ...
                {/* Uploaded Files */}
                <div className="bg-gray-900 border border-gray-700 rounded-lg p-6">
                  <h3 className="text-lg font-semibold text-white mb-4">Knowledge Base Files</h3>
                  <div className="space-y-3">
                    {formData.uploadedFiles.map(file => (
                      <div key={file.id} className="flex items-center justify-between p-3 bg-gray-800 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          {file.type === 'video' && <Video className="w-4 h-4 text-primary-400" />}
                          {file.type === 'document' && <FileText className="w-4 h-4 text-red-400" />}
                    ...
                    {isEditing && (
                      <div className="mt-4">
                        ...
                        <label
                          ...
                          className={`inline-flex items-center gap-2 px-4 py-2 border-2 border-dashed border-gray-600 rounded-lg hover:border-primary-600 transition cursor-pointer text-gray-300 ${
        ...
        {/* Footer */}
        {isEditing && (
          <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-700 bg-gray-800 rounded-b-xl">
            <button
              onClick={() => {
                setIsEditing(false);
            ...
            <button
              onClick={handleSave}
              className="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg transition flex items-center gap-2"
            >
